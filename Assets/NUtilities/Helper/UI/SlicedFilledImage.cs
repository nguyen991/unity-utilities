...
namespace NUtilities.Helper.UI
{
#if UNITY_EDITOR
    using UnityEditor;
                    ...
                    case FillDirection.Down:
                        {
                            ...
                            {
                                s_SlicedVertices[1].y =
                                    rect.height * (1f - m_FillAmount) + border.y;
                                s_SlicedVertices[2].y = rect.height - border.w;
                            }
                        }
                        break;
            ...
            for (int x = 0; x < 3; x++)
            {
                ...
                for (int y = 0; y < 3; y++)
                {
                    ...
                    switch (m_FillDirection)
                    {
                        ...
                        case FillDirection.Left:
                            sliceStart =
                                1f - (s_SlicedVertices[x2].x - rectStartPos) * _1OverTotalSize;
                            sliceEnd =
                                1f - (s_SlicedVertices[x].x - rectStartPos) * _1OverTotalSize;
                            break;
                        case FillDirection.Down:
                            sliceStart =
                                1f - (s_SlicedVertices[y2].y - rectStartPos) * _1OverTotalSize;
                            sliceEnd =
                                1f - (s_SlicedVertices[y].y - rectStartPos) * _1OverTotalSize;
                            break;
        ...
        bool ICanvasRaycastFilter.IsRaycastLocationValid(Vector2 screenPoint, Camera eventCamera)
        {
            ...
            for (int i = 0; i < 2; i++)
            {
                ...
                float lerp = Mathf.InverseLerp(
                    adjustedBorder[i],
                    rect.size[i] - adjustedBorder[i + 2],
                    local[i]
                );
            ...
            Vector2 normalized = new Vector2(
                local.x / textureRect.width,
                local.y / textureRect.height
            );

            // Convert to texture space.
            float x =
                Mathf.Lerp(textureRect.x, textureRect.xMax, normalized.x)
                / activeSprite.texture.width;
            float y =
                Mathf.Lerp(textureRect.y, textureRect.yMax, normalized.y)
            ...
            try
            {
                return activeSprite.texture.GetPixelBilinear(x, y).a
